...
public abstract class AbstractConfig {
	...
	/**
	 * 获取值，如果options和properties都没有，则取默认值defaultVal
	 * @param name
	 * @param options
	 * @param properties
	 * @param defaultVal
	...
	protected MaxwellMysqlConfig parseMysqlConfig(String prefix, OptionSet options, Properties properties) {
		...
		config.replicationHost     = fetchOption(prefix + "host", options, properties, null);
		config.replicationPassword = fetchOption(prefix + "password", options, properties, null);
		config.replicationUser     = fetchOption(prefix + "user", options, properties, null);
		config.replicationPort     = Integer.valueOf(fetchOption(prefix + "port", options, properties, "3306"));
