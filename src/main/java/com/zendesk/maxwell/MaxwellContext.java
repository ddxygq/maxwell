...
public class MaxwellContext {
	...
	private BinlogBootstrapEvent bootstrapEvent ;
	...
	public MaxwellContext(MaxwellConfig config) throws SQLException, URISyntaxException {
		...
		this.replicationConnectionPool = new ConnectionPool("ReplicationConnectionPool", 10, 0, 10,
				config.replicationMysql.getReplicationConnectionURI(false), config.replicationMysql.replicationUser, config.replicationMysql.replicationPassword);
		...
		LOGGER.info("MaxwellContext-->initPosition:{},replayMode:{}",this.config.initPosition,this.config.replayMode);
	...
	public void start() throws IOException {
		...
		getPositionStoreThread(); // boot up thread explicitly.显式启动线程
	}
...
//		LOGGER.info("maxwell --> getPositionStoreThread start");
		...
		if(null != this.replicator){
			if(null == this.bootstrapEvent){
//				LOGGER.info("创建BinlogBootstrapEvent对象，时间：{}",new Date().getTime());
				this.bootstrapEvent = new BinlogBootstrapEvent(this, (BinlogConnectorReplicator) this.replicator);
			}
//			LOGGER.info("调用BinlogBootstrapEvent对象，时间：{}",new Date().getTime());
			this.positionStoreThread.setBinlogBootstrapEvent(bootstrapEvent);
		}
	...
	public Position getInitialPosition() throws SQLException {
		LOGGER.info("getInitialPosition-->Position:{}",this.initialPosition);
