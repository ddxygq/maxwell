...
public class AsynchronousBootstrapper extends AbstractBootstrapper {
	...
	public void startBootstrap(final RowMap bootstrapStartRow, final AbstractProducer producer, final Replicator replicator) throws Exception {
		if (thread == null) {
			...
			LOGGER.info("Asynchronous-->startBootstrap synch-->bootstrappedRow:{}",bootstrappedRow.toString());
			...
			LOGGER.info("Asynchronous-->startBootstrap queueRow-->thread:{},bootstrapStartRow:{}",thread.toString(),bootstrapStartRow.toString());
			queueRow(bootstrapStartRow);
		}
	}
	@Override
	public void startBootstrapForSeparate(final RowMap bootstrapRow, final AbstractProducer producer, final Replicator replicator) throws Exception {
//		LOGGER.info("Asynchronous-->startBootstrap synch-->bootstrappedRow:{}",bootstrapRow.toString());
		Thread threadBootstrap = new Thread(new Runnable() {
			@Override
			public void run() {
				try {
					synchronousBootstrapper.startBootstrap(bootstrapRow, producer, replicator);
				}catch ( Exception e ) {
					e.printStackTrace();
					System.exit(1);
				}
...
//			LOGGER.info(String.format("async bootstrapping ended for %s.%s", databaseName, tableName));
	...
	@Override
	public void completeBootstrapForSeparate(RowMap bootstrapCompleteRow, AbstractProducer producer, Replicator replicator) throws Exception {
		this.completeBootstrap(bootstrapCompleteRow, producer, replicator);
	}
...
//		LOGGER.info("async-->resume-->producer:{},replicator:{}",producer.toString(),null!=replicator.getRow() ? replicator.getRow():"null");
...
//			LOGGER.info("async-->startBootstrap-->row:{},producer:{},replicator:{}",row.getData(),producer,null!=replicator.getRow()?replicator.getRow():"null");
			startBootstrap(row, producer, replicator);
		} else if ( isCompleteBootstrapRow(row) ) {
//			LOGGER.info("async-->completeBootstrap-->row:{},producer:{},replicator:{}",row.getData(),producer,null!=replicator.getRow()?replicator.getRow():"null");
	...
	public void workForBootstrape(RowMap row, AbstractProducer producer, Replicator replicator) throws Exception {
		if ( isStartBootstrapRow(row) ) {
//			LOGGER.info("async-->startBootstrapForSeparate-->row:{},producer:{},replicator:{}",row.getData(),producer,null!=replicator.getRow()?replicator.getRow():"null");
			startBootstrapForSeparate(row, producer, replicator);
		} else if ( isCompleteBootstrapRow(row) ) {
//			LOGGER.info("async-->completeBootstrapForSeparate-->row:{},producer:{},replicator:{}",row.getData(),producer,null!=replicator.getRow()?replicator.getRow():"null");
			completeBootstrapForSeparate(row, producer, replicator);
		}
	}
}
