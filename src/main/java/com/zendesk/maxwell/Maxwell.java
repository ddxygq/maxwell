...
public class Maxwell implements Runnable {
	...
	protected Position getInitialPosition() throws Exception {
		...
		LOGGER.info("初始化节点1-->Position:{}",initial);
		if (initial == null) {

			/* second method: are we recovering from a master swap? 从主交换中恢复*/
			if ( config.masterRecovery )
				initial = attemptMasterRecovery();
			LOGGER.info("初始化节点2-->Position:{}",initial);
	...
	private void logBanner(AbstractProducer producer, Position initialPosition) {
		String producerName = producer.getClass().getSimpleName();//获取简写名称
	...
	public static void main(String[] args) {
		try {
			...
			LOGGER.info("main-->Position:{}",config.initPosition);
